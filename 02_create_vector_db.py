...
# document_to_embed = "knowledge_pool/Austin.txt"
def get_embedding(text, model=embedding_model):
   text = text.replace("\n", " ")
   return local_client.embeddings.create(input = [text], model=model).data[0].embedding

documents_dir = "knowledge_pool"

for document in os.listdir(documents_dir):
    # Iterate through the txt files
    if document.endswith(".txt"):
        document_to_embed = os.path.join(documents_dir, document)
        print(f"Processing {document_to_embed}")
